...
categories: [PWN]
---
堆的UAF漏洞是一个重要的漏洞，即释放后使用，指程序使用free等函数释放了堆内存后，没有将指向堆的指针置空（设为NULL），从而导致依然可以利用该指针去对释放的内存进行读写，最终导致信息泄漏、控制流劫持等。UAF可以说是堆漏洞利用的核心，它给了我们像栈溢出一样的，读写内存的能力，许多提权漏洞都基于UAF。
