...
categories: [PWN]
---
ret2shellcode是一种基础的栈溢出利用技术，核心思路是构造恶意的机器指令并注入到程序内存，然后通过覆盖返回地址，返回到恶意的代码段执行。由于执行代码，需要该内存区域具有可读可写可执行（RWX）三种权限，而现代操作系统会限制程序段的RWX最多只能同时具有两种权限，因此该方法实际已经没法起作用，学习时可以找低版本的老系统进行测试。接下来以一道题目看看ret2shellcode的利用。
